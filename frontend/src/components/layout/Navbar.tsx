  ...
  return (
    <>
      <nav className="bg-[#0a0a0a] fixed top-0 w-full z-50 transition-all duration-500 ease-in-out border-b border-gray-900 shadow-2xl">
        <div className={`max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 transition-all duration-500 ${isScrolled ? 'py-4 md:py-5' : 'pt-8 pb-4'}`}>
          
          {/* Top Centered Logo Section */}
          <div className={`flex justify-center items-center w-full relative transition-all duration-500 overflow-hidden ${isScrolled ? 'h-0 mb-0 opacity-0' : 'h-16 md:h-20 mb-8 opacity-100'}`}>
          ...
          <div className="flex justify-between items-center">
            ...
            <div className="hidden md:flex w-full justify-center space-x-8 lg:space-x-12">
              {navigation.map((item) => (
                <Link
                  key={item.name}
                  href={item.href}
