  ...
  useEffect(() => {
    const fetchTestimonials = async () => {
      try {
        ...
        const items = response.data?.data?.testimonials;
        if (Array.isArray(items) && items.length > 0) {
          setTestimonials(
            items.map((item: { quote: string; name: string; designation: string; photoUrl?: string }) => ({
              text: item.quote,
              author: item.name,
        ...
        <div className="relative bg-[#111] rounded-none p-10 md:p-20 shadow-2xl border border-gray-900 overflow-hidden">
          ...
          <div className="text-center min-h-[300px] flex flex-col justify-center relative z-10 mt-6">
            ...
            <div className="flex flex-col items-center gap-4">
              ...
              <div>
                <h4 className="text-xl font-bold text-[#f26522] uppercase tracking-widest">{current.author}</h4>
                <p className="text-gray-500 font-medium mt-2 text-sm uppercase tracking-widest">{current.title}</p>
              </div>
            </div>
          </div>
