...
export const ImpactStatistics: React.FC<ImpactStatisticsProps> = ({ content }) => {
  ...
  const description =
    content?.description ||
    'Beyond the numbers lies a deeper story of human potential. For over two decades, Sajan Shah has been a catalyst for change, bridging the gap between average performance and extraordinary results.';
  const ctaText = content?.ctaText || 'View Impact Report';
  const ctaLink = content?.ctaLink || '/contributions';
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-16 items-start">
          <div className="lg:col-span-4">
            ...
            <p className="text-gray-400 text-lg font-light leading-relaxed mb-12">{description}</p>
            <a href={ctaLink} className="group flex items-center gap-4 text-[#f26522] font-bold uppercase tracking-widest text-xs">
              <span>{ctaText}</span>
          ...
          <div className="lg:col-span-8 grid grid-cols-1 sm:grid-cols-2 gap-x-12 gap-y-16">
            {stats.map((stat, idx) => (
              <div key={idx} className="relative">
                ...
                <h3 className="text-[#f26522] font-bold text-xs uppercase tracking-[0.2em] mb-3">{stat.label}</h3>
                <p className="text-gray-500 text-sm font-light">{stat.desc}</p>
        ...
        <div className="mt-32 pt-16 border-t border-gray-900 flex flex-col md:flex-row justify-between items-center gap-8">
          <p
            ...
            dangerouslySetInnerHTML={{
              __html:
                content?.bottomBannerText1 ||
                'Ready to join the <span class="text-white font-medium">16 Million+</span> transformation journey?',
            }}
          />
          <p
            className="text-xl md:text-2xl font-light text-gray-300"
            dangerouslySetInnerHTML={{
              __html:
                content?.bottomBannerText2 ||
                'Every number is a <span class="text-white font-medium">Story</span> of change',
            }}
          />
          <p
            className="text-xl md:text-2xl font-light text-gray-300"
            dangerouslySetInnerHTML={{
              __html:
                content?.bottomBannerText3 ||
                'Every effort is a <span class="text-white font-medium">step towards</span> a better future.',
            }}
          />
          <a
            href={content?.bottomCtaLink || '/contact'}
          ...
          >
            {content?.bottomCtaText || 'join the Community'}
          </a>
        </div>
      </div>
    </section>
