...
function formatTicketPrice(event: SajanEvent) {
  ...
  if (event.isFree !== false) return 'REGISTER FREE';
  return 'REGISTER NOW';
}

function formatPriceLabel(event: SajanEvent) {
...
function formatPlace(event: SajanEvent) {
  if (event.format === 'Online' || (event.city || '').toLowerCase() === 'online') {
    return event.tag && event.tag !== 'Corporate' ? event.tag : 'Zoom Virtual';
  }
  return event.city || event.tag || 'TBA';
}

export default function EventsWebinars({ events }: EventsWebinarsProps) {
  const [showAll, setShowAll] = useState(false);
  const allUpcoming = events;
  const visible = showAll ? allUpcoming : allUpcoming.slice(0, 2);
  const fallbackBgs = ['/Sir Event3.jpeg', '/sir Event2.jpeg', '/Sir Event4.jpeg'];

  if (allUpcoming.length === 0) {
    return (
      <section id="webinars" className="py-24 px-4 md:px-8 max-w-7xl mx-auto border-t border-white/5">
        <h2 className="text-5xl md:text-6xl font-black uppercase tracking-normal mb-8 leading-[1.1]">
          ...
          <span className="text-brand-orange">Events</span>
        </h2>
    ...
    <section id="webinars" className="py-24 px-4 md:px-8 max-w-7xl mx-auto border-t border-white/5 relative overflow-hidden">
      ...
      <div className="flex flex-col md:flex-row justify-between items-end mb-24 relative z-10">
        <div>
          ...
          <p className="text-gray-400 text-lg max-w-md">
            Join sessions from anywhere and unlock your full potential. Latest upcoming event appears first.
          </p>
        </div>
      </div>
      ...
      <div className="space-y-12 relative z-10">
        {visible.map((event, index) => {
          const bgSrc = resolveMediaUrl(event.thumbnail) || fallbackBgs[index % fallbackBgs.length];
          const plainDesc = (event.topic || '').replace(/<[^>]*>/g, '').trim();
          ...
          const DEFAULT_CARD_EYEBROW = 'Discover transformation with Sajan Shah';
          // First line (if short) can act as orange eyebrow; rest/full text is the card body
          const lines = plainDesc.split(/\n+/).map((l) => l.trim()).filter(Boolean);
          const firstLine = lines[0] || '';
          const useSplitTagline = lines.length > 1 && firstLine.length > 0 && firstLine.length <= 80;
          const topicLine = useSplitTagline
            ? firstLine.replace(/^#\s*/, '')
            : DEFAULT_CARD_EYEBROW;
          const bodyText = useSplitTagline
            ? lines.slice(1).join(' ')
            : plainDesc || DEFAULT_CARD_BODY;
            ...
            >
              ...
              <div className="flex-1 p-8 md:p-14 relative z-20 flex flex-col justify-center opacity-60 group-hover:opacity-100 transition-opacity duration-500 drop-shadow-2xl">
                <div className="flex items-center space-x-3 text-brand-orange font-bold text-xs tracking-[0.2em] uppercase mb-6">
                  ...
                  <span className="line-clamp-2">{topicLine}</span>
                </div>
                ...
                <div className="flex flex-wrap items-center gap-x-10 gap-y-6 mt-auto">
                  ...
                  <div className="flex gap-10">
                    <div>
                      ...
                      <div className="font-bold text-sm text-gray-200">{format(event.date, 'do MMM yyyy')}</div>
                    </div>
                    <div>
                      <div className="text-[10px] text-gray-500 uppercase font-black tracking-widest mb-1">Time</div>
                      <div className="font-bold text-sm text-gray-200">{format(event.date, 'HH:mm')}</div>
                    </div>
                    <div>
                      <div className="text-[10px] text-gray-500 uppercase font-black tracking-widest mb-1">Place</div>
                      <div className="font-bold text-sm text-gray-200">{formatPlace(event)}</div>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="w-full md:w-[350px] relative bg-black/30 backdrop-blur-sm p-8 md:p-10 flex flex-col justify-center border-t md:border-t-0 md:border-l border-white/5 opacity-60 group-hover:opacity-100 transition-opacity duration-500 overflow-hidden">
                <div className="absolute inset-0 opacity-[0.06] pointer-events-none z-0 flex items-center justify-center overflow-hidden">
                  {/* eslint-disable-next-line @next/next/no-img-element */}
                  <img src="/sir sign.png" alt="" className="w-full h-full object-cover object-center rotate-[-15deg] scale-125" />
                </div>
                ...
                <div className="relative z-10">
                  <div className="text-center mb-6">
                    ...
                    <p className="mt-3 text-lg font-black text-white tracking-wide">
